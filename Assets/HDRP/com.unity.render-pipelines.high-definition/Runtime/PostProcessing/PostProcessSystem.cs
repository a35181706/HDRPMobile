...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    ...
    public sealed class PostProcessSystem
    {
        ...
        static void GrabCoCHistory(HDCamera camera, out RTHandle previous, out RTHandle next)
        {
            RTHandle Allocator(string id, int frameIndex, RTHandleSystem rtHandleSystem)
            {
                return rtHandleSystem.Alloc(
                    ...
                    colorFormat: UnityEngine.Experimental.Rendering.HDPipeline.HDRenderPipeline.OverrideRTGraphicsFormat(GraphicsFormat.R16_SFloat), enableRandomWrite: true, xrInstancing: true, name: "CoC History"
                );
            }
        ...
        void DoMotionBlur(CommandBuffer cmd, HDCamera camera, RTHandle source, RTHandle destination)
        {
            ...
            bool scattering = SystemInfo.IsFormatSupported(UnityEngine.Experimental.Rendering.HDPipeline.HDRenderPipeline.OverrideRTGraphicsFormat(GraphicsFormat.R32_UInt), FormatUsage.LoadStore) && SystemInfo.IsFormatSupported(UnityEngine.Experimental.Rendering.HDPipeline.HDRenderPipeline.OverrideRTGraphicsFormat(GraphicsFormat.R16_UInt), FormatUsage.LoadStore);
            ...
            RTHandle preppedMotionVec = m_Pool.Get(Vector2.one, HDRenderPipeline.OverrideRTGraphicsFormat(GraphicsFormat.B10G11R11_UFloatPack32));
