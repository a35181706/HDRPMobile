    ...
    public class HDRenderPipeline : UnityEngine.Rendering.RenderPipeline
    {
        ...
        void InitializeRenderTextures()
        {
            ...
            if(m_Asset.currentPlatformRenderPipelineSettings.lowresTransparentSettings.enabled)
            {
                ...
                m_LowResTransparentBuffer = RTHandles.Alloc(Vector2.one * 0.5f, filterMode: FilterMode.Point, colorFormat: UnityEngine.Experimental.Rendering.HDPipeline.HDRenderPipeline.OverrideRTGraphicsFormat(GraphicsFormat.R16G16B16A16_SFloat), enableRandomWrite: true, xrInstancing: true, useDynamicScale: true, name: "Low res transparent");
            }
            ...
            if (Debug.isDebugBuild)
            {
                ...
                m_DebugFullScreenTempBuffer = RTHandles.Alloc(Vector2.one, filterMode: FilterMode.Point, colorFormat: HDRenderPipeline.OverrideRTGraphicsFormat(GraphicsFormat.R16G16B16A16_SFloat), xrInstancing: true, useDynamicScale: true, name: "DebugFullScreen");
...

            {
                    ...
                    if (size != Vector2.zero)
                    {
                        ...
                        if (m_TemporaryTargetForCubemaps == null)
                        {
                            m_TemporaryTargetForCubemaps = new RenderTexture(
                                size.x, size.y, 1, UnityEngine.Experimental.Rendering.HDPipeline.HDRenderPipeline.OverrideRTGraphicsFormat(GraphicsFormat.R16G16B16A16_SFloat)
                            )
                            {
                ...
                using (ListPool<int>.Get(out List<int> renderRequestIndicesToRender))
                {
                    ...
                    for (int i = renderRequestIndicesToRender.Count - 1; i >= 0; --i)
                    {
                        ...
                        ExcuteCameraEvent(CameraEvent.AfterEverything, renderContext, renderRequest.hdCamera);
        ...
        void ExecuteRenderRequest(RenderRequest renderRequest, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            else
            {
                ...
                ExcuteCameraEvent(CameraEvent.BeforeSkybox, renderContext, hdCamera);
                RenderSky(hdCamera, cmd);
                ExcuteCameraEvent(CameraEvent.AfterSkybox, renderContext, hdCamera);
            ...
            switch (hdCamera.frameSettings.litShaderMode)
            {
                case LitShaderMode.Forward:
                    using (new ProfilingSample(cmd, "Depth Prepass (forward)", CustomSamplerId.DepthPrepass.GetSampler()))
                    {
                        ...
                        ExcuteCameraEvent(CameraEvent.BeforeDepthTexture, renderContext, hdCamera);
                        ...
                        ExcuteCameraEvent(CameraEvent.AfterDepthTexture, renderContext, hdCamera);
                    }
                    break;
        ...
        void RenderGBuffer(CullingResults cull, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            using (new ProfilingSample(cmd, m_CurrentDebugDisplaySettings.IsDebugDisplayEnabled() ? "GBuffer Debug" : "GBuffer", CustomSamplerId.GBuffer.GetSampler()))
            {
                ...
                ExcuteCameraEvent(CameraEvent.BeforeGBuffer, renderContext, hdCamera);
                ...
                ExcuteCameraEvent(CameraEvent.AfterGBuffer, renderContext, hdCamera);
            ...
            using (new ProfilingSample(cmd, m_ForwardPassProfileName, CustomSamplerId.ForwardPassName.GetSampler()))
            {
                ...
                else
                {
                    ...
                    if (pass == ForwardPass.Opaque)
                    {
						...
						ExcuteCameraEvent(CameraEvent.BeforeForwardOpaque, renderContext, hdCamera);
                        RenderOpaqueRenderList(cullResults, hdCamera, renderContext, cmd, passNames, m_currentRendererConfigurationBakedLighting);

                        ExcuteCameraEvent(CameraEvent.AfterForwardOpaque, renderContext, hdCamera);
                    }
                    else
                    {
                        ...
                        if (pass == ForwardPass.Transparent)
                        {
                            ExcuteCameraEvent(CameraEvent.BeforeForwardAlpha, renderContext, hdCamera);
                        }
                        ...
                        if (pass == ForwardPass.Transparent)
                        {
                            ExcuteCameraEvent(CameraEvent.AfterForwardAlpha, renderContext, hdCamera);
                        }
                    }
                }
            }
        ...
        private static void ExcuteCameraEvent(CameraEvent camEvent,ScriptableRenderContext context,HDCamera hdCamera)
        {
            if (null == hdCamera || null == context)
            {
                return;
            }

            Camera cam = hdCamera.camera;

            CommandBuffer[] allBuffers = cam.GetCommandBuffers(camEvent);
            foreach (CommandBuffer buffer in allBuffers)
            {
        ...
        public static GraphicsFormat OverrideRTGraphicsFormat(GraphicsFormat source)
        {
            ...
            //Debug.LogError("��֧�ֵ�ƽ̨:" + Application.platform + ",��ʽ:" + source);
            //return source;
        }
