...

            {
                ...
                using (ListPool<int>.Get(out List<int> renderRequestIndicesToRender))
                {
                    ...
                    for (int i = renderRequestIndicesToRender.Count - 1; i >= 0; --i)
                    {
                        ...
                        UnityEngine.Rendering.CommandBuffer[] allbuffers = Camera.main.GetCommandBuffers(UnityEngine.Rendering.CameraEvent.AfterEverything);

                        foreach (UnityEngine.Rendering.CommandBuffer buffers in allbuffers)
                        {
                            renderContext.ExecuteCommandBuffer(buffers);
                        }
